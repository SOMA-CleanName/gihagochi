...
어드민이 이걸 **되읽으려면** 같은 Apps Script 프로젝트에 `doGet`을 추가하면 된다.
...
admin 배포 환경(Vercel 등)에 추가 — **서버 전용**(NEXT_PUBLIC 아님):

| 변수 | 값 |
|---|---|
| `SHEETS_READ_URL` | Apps Script 웹앱 배포 URL |
| `SHEETS_READ_TOKEN` | 위에서 정한 `READ_TOKEN`과 동일 |

설정 후 `/landing` 새로고침 → 데이터 표시.
